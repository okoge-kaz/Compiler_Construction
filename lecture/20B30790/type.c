...
static void set_arg_types(struct AST *ast_plist, struct Type *type) {
    struct AST *ast, *ast_par_decn;
    int i, arg_num;

    assert(!strcmp(ast_plist->ast_type, "AST_parameter_list_single") || !strcmp(ast_plist->ast_type, "AST_parameter_list_pair"));
...
struct Type *type_add(struct Type *type1, struct Type *type2) {
    if ((type1->kind == TYPE_KIND_POINTER) && (type2->kind == TYPE_KIND_POINTER)) {
...
struct Type *type_uop(struct AST *ast, struct Type *type1) {
    if (!strcmp(ast->ast_type, "AST_unary_operator_plus") || !strcmp(ast->ast_type, "AST_unary_operator_minus") || !strcmp(ast->ast_type, "AST_unary_operator_negative")) {
        return types.t_long;
